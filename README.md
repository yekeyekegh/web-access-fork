...
● 有两种"更新"场景，需要区分：
  ...
  这样你的自定义修改会和上游更新合并，冲突由 git 处理。

  2. 不要用 Claude Code 的 skill 安装/更新命令

  如果你执行类似 claude skill install ... 或重新安装 skill，它会覆盖整个目录，你的 .git/ 和所有自定义修改都会丢失。

  万一不小心覆盖了，恢复方式：
  cd ~/.claude/skills/web-access
  git init
  git remote add origin https://github.com/yekeyekegh/web-access-fork.git
  git fetch origin
  git checkout -f origin/main

  总结：只通过 git fetch upstream + merge 更新，不走 Claude Code 的 skill 安装机制。
