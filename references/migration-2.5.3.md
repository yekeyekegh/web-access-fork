...
> **本 fork 注意**：本仓库的 proxy 端点（除 `/health`）需要鉴权 token，URL 走 POST body、token 仍走 query。完整写法为
> `curl -s -X POST --data-raw 'https://example.com' "http://localhost:3456/new?token=$TOKEN"`
> 下文示例为对齐上游而省略了 `&token=$TOKEN`，实际调用时请按此补上。
...
> **† 本 fork 的「新」列示例为对齐上游而省略了 `&token=$TOKEN`。** 本仓库除 `/health` 外所有端点需鉴权，实际调用须把 token 加到 query，例如
> `curl -X POST --data-raw 'https://example.com' "http://localhost:3456/new?token=$TOKEN"`。请勿直接照搬表中行，否则会收到 `403`。
...
A: 收到 HTTP 400 响应，body 中包含 `error`、`migration` 文档路径、`example` 示例。按提示改 POST 调用即可。注意：本 fork 的鉴权检查先于方法检查——若调用**未带 token**，会先收到 `403`（认证失败），补上 `?token=$TOKEN` 后才会看到这个 400 迁移提示。
